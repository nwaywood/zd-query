...
func TestBuildOrgCaches(t *testing.T) {
	expectedOrgMap := OrgMap{"1": orgs[0], "2": orgs[1], "3": orgs[2]}
	expectedIndexMap := IndexTypeMap{
	...
	assert.Equal(t, expectedIndexMap, indexMap)
}

func TestBuildUserCaches(t *testing.T) {
	expectedUserMap := UserMap{"101": users[0], "102": users[1]}
	expectedIndexMap := IndexTypeMap{
...
func TestBuildTicketCaches(t *testing.T) {
	expectedTicketMap := TicketMap{"t1": tickets[0], "t2": tickets[1], "t3": tickets[2]}
	expectedIndexMap := IndexTypeMap{
